...
function M.run()
  ...
  new_buffer({ "''" }, { 1, 1 })
  assert_equal(softpair.backspace(), "<BS><Del>", "single quote pair backspace")

  new_buffer({ "``" }, { 1, 0 })
  assert_equal(softpair.delete_forward(), "<Del><Del>", "backtick pair forward delete")
  ...
  new_buffer({ "{ [ (x) ] }" }, { 1, 5 })
  set_point(5)
  assert_truthy(softpair.copy_inner("{"), "copy inner searches upward")
  assert_equal(vim.fn.getreg('"'), " [ (x) ] ", "copy inner upward register")
  ...
  new_buffer({ "((a) b)" }, { 1, 1 })
  set_point(1)
  assert_truthy(softpair.squeeze(), "squeeze on opening delimiter uses current list")
  assert_equal(current_text(), "( b)", "squeeze on opening delimiter result")
  assert_equal(vim.fn.getreg('"'), "a", "squeeze on opening delimiter register")
  ...
  new_buffer({ "あb" }, { 1, 0 })
  set_point(0)
  assert_truthy(softpair.forward_delete_char(), "forward char deletes utf-8 char")
  assert_equal(current_text(), "b", "forward utf-8 delete keeps byte boundaries")

  new_buffer({ "あbc" }, { 1, 3 })
  set_point(3)
  assert_truthy(softpair.backward_delete_char(), "backward char deletes utf-8 char")
  assert_equal(current_text(), "bc", "backward utf-8 delete keeps byte boundaries")

  new_buffer({ "ab" }, { 1, 1 })
  set_point(1)
  assert_truthy(softpair.forward_delete_char(), "normal eol forward char deletes current byte")
  assert_equal(current_text(), "a", "normal eol forward delete result")
  ...
  new_buffer({ "alpha", "beta", "gamma" }, { 1, 0 })
  vim.fn.setpos("'<", { 0, 1, 1, 0 })
  vim.fn.setpos("'>", { 0, 2, 1, 0 })
  assert_truthy(softpair.kill_active_region("V"), "linewise visual kill")
  assert_equal(current_text(), "gamma", "linewise visual kill result")
  assert_equal(vim.fn.getregtype('"'), "V", "linewise visual kill register type")

  new_buffer({ "alpha", "beta" }, { 1, 0 })
  vim.fn.setpos("'<", { 0, 1, 1, 0 })
  vim.fn.setpos("'>", { 0, 2, 2, 0 })
  assert_falsy(softpair.kill_active_region("\22"), "blockwise visual kill is refused")
  assert_equal(current_text(), "alpha\nbeta", "blockwise visual kill keeps text")
